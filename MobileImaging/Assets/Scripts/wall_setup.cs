			...
			for (int i = 0; i < ar_objects.Length; i++) {
				...
				for (int j = 0; j < i; j++) {
					...
					if (enabledi && enabledj) {
						...
						Vector3 tmp_pos = obji.transform.position - objj.transform.position;
						// InverseTransformDirection : Transforms a direction from world space to local space.
						relative_pos [i, j] = objj.transform.InverseTransformDirection (tmp_pos);
			...
			for (int i = 1; i < scene_objects.Length; i++) {
				...
				for (int j = 0; j < i; j++) {
					if (rel_rec [i, j] > 0) {
						...
						// InverseTransformDirection : Transforms a direction from world space to local space.
						result_pos += objj.transform.position + objj.transform.TransformDirection (rel_pos);
						add_count++;
