...
//some stuff heres
//Note: all vectors which indicate a direction must be normalized 
public class Math3d : MonoBehaviour {
	...
	//Fast code which converts a string to an integer. Make sure the string is a valid int.
	public static int IntParseFast(string text){
	...
	//Get the index of the array of the number to be found. If the number is not found,
	//it returns the index of the first available empty slot (value of -1) as well.
	public static void GetNumberIndex(out int indexNumber, out int indexEmptySlot,  int[] array, int number){
	
		indexNumber = -1;
		indexEmptySlot = -1;

		//find the number in the slots
	...
	public static bool CompareBoolArray(bool[] array1, bool[] array2){
